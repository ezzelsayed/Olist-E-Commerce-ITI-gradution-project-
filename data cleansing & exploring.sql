...
-------------------------------------------------------------------------------------------------------
-- Adding Names To Seller_dim (After Adding Table with rondom names And Gender in DB)
Select temp.seller_id,temp2.first_name,temp2.L_name,temp2.gender
into New_Seller_Dim
                      from(
                      Select seller_id , ROW_NUMBER() over(order by seller_id) as RN1
                      From Seller_Dim) as temp,
					  ( 
					    select F_name first_name ,L_name ,gender,ROW_NUMBER() over(order by F_name) as RN2  
						from seller_name) as temp2
Where RN1=RN2
-------------------------------------------------------------------------------------------------------
--Adding Names to Customer_Dim (After Adding Table with rondom names And Gender in DB)
Select tem.customer_id,tem.customer_unique_id,tem2.F_name,tem2.L_name,tem2.gender
Into New_Customer_Dim 
					 from(
                      Select customer_id,customer_unique_id , ROW_NUMBER() over(order by Customer_id) as RN1
                      From Customer_Dim ) as tem,
					  ( 
					    select F_name ,L_name ,gender,ROW_NUMBER() over(order by F_name) as RN2  
						from [dbo].[Customer_Name] ) as tem2
Where RN1=RN2
--------------------------------------------------------------------------------------------------------
